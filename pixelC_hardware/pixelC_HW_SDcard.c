...
#include"pixelC_HW_SDcard.h"

u8  SD_Type=0;//SD卡的类型 
//############################【移植修改区】#############################

void SPI1_Init(void)										   //SPI 口初始化
{
...
void SPI1_SetSpeed(u8 SPI_BaudRatePrescaler)						//SPI 速度设置函数
{
	...
	SPI1->CR1&=0XFFC7; 
	SPI1->CR1|=SPI_BaudRatePrescaler<<3;	//设置SPI2速度  
	SPI1->CR1|=1<<6; 		//SPI设备使能
}
...
void SD_SPI_Init(void)
{
	...
	#if CS_Pin/8		//CK							
			CS_GPIOx->CRH&=~((u32)0xf<<((CS_Pin%8)*4));		
			CS_GPIOx->CRH|=(u32)0x3<<((CS_Pin%8)*4);
	#else
			CS_GPIOx->CRL&=~((u32)0xf<<((CS_Pin%8)*4));
			CS_GPIOx->CRL|=(u32)0x3<<((CS_Pin%8)*4);
	#endif																		 
	PAout(4)=1;			  //上拉
}
...
{			  	  
    ...
    while(len--)//开始接收数据
    {
        *buf=SD_SPI_ReadWriteByte(0xFF);//提高速度,减少函数传参时间
        buf++;
    }
    //下面是2个伪CRC（dummy CRC）
